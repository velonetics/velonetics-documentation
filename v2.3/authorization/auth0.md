...
description: Control access to Velonetics endpoints by authenticating users on Auth0 and authorizing them or not on Velonetics.
weight: 100
menu:
  community_v2.3:
...
docker run --rm -v "$PWD:/etc/velonetics" -p "8080:8080" {{< product image >}}:2.3
{{< /terminal >}}
