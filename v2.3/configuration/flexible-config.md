...
The activation of the Flexible Configuration requires injecting the **environment variable** `FC_ENABLE=1` when running a velonetics `run` or `check` command. Still, you can use **additional variables** depending on the features you'd like to enable.
...
- `FC_OUT=file.json`: Saves the resulting configuration after rendering the template. It's required when you don't use JSON content (e.g., `FC_OUT=velonetics.yml`), or when you need to pass the output to another program (like [`check --lint`](/docs/v2.3/configuration/check/)).
...
#### Sprig functions
Complementing the [Go built-in template language](http://golang.org/pkg/text/template/), and the Velonetics functions, the templates also adds more than 100 commonly used functions.
...
As the configuration is now composed of several pieces, it's easy to make a mistake at some point. Test the syntax of all the files with the `velonetics check` command and pay attention to the output to verify there aren't any errors.
...
Save the following `docker-compose.yml` and do a `docker compose up`. This setup with the [`:watch` image](/docs/v2.3/developer/hot-reload/) will allow you to work locally with the latest version of Velonetics and apply the changes automatically every time you change a source file.
...
Let's imagine your `velonetics.tmpl` looks like this:
...
FC_SETTINGS="$PWD/config/settings" \
FC_PARTIALS="$PWD/config/partials" \
velonetics check -t -d -c "$PWD/config/velonetics.tmpl"
{{< /terminal >}}
