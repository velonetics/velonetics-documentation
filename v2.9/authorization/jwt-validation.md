...
3) **If you don't have an identity server**, you can still use your classic monolith/backend login system and adapt it to return a JWT payload (a simple JSON). From here, let Velonetics [sign the token for you](/docs/v2.9/authorization/jwt-signing/) and start using tokens immediately.
...
- The `jwk_url` must be accessible by Velonetics at all times ([caching is recommended](/docs/v2.9/authorization/jwk-caching/))
...
The [Velonetics Playground](/docs/v2.9/overview/playground/) demonstrates how to protect endpoints using JWT and includes two examples ready to use:
