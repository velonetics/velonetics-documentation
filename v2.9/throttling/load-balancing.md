...
Velonetics's egress load balancer acts at the [backend level](/docs/v2.9/backends/) and manages the connections to the backends from within the gateway.
...
The host list does not have any monitoring by Velonetics at this level and **it won't remove entries from it** if the backend fails. To control failures in your backends, you should use the [Circuit Breaker](/docs/v2.9/backends/circuit-breaker/).
