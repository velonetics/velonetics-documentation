...
The `velonetics check` command **validates Velonetics configuration files** written in any of its [supported formats](/docs/v2.12/configuration/supported-formats/).
...
- `-l` or `--lint` (*optional*) to check that your configuration file is properly linted and does not contain unrecognized options or wrong types. This option downloads the schema from `https://www.velonetics.io/schema/v2.12/velonetics.json` every time (each version uses its schema), but use the `-n` flag to use the embedded schema (see below). **This flag is the most important one** and you should always have it in your pipeline.
