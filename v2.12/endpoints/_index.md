...
If you are still getting familiar with Velonetics's configuration structure, take a moment to read [Understanding the configuration file](/docs/v2.12/configuration/structure/). This section explains what an `endpoint` looks like, but take a moment to understand the different pieces.
{{< /note >}}
...
By default Velonetics only works with **RESTful URL patterns** in its endpoint definition. Enable the option [`disable_rest`](/docs/v2.12/service-settings/http-server-settings/#disable_rest) in the root of your configuration if need unrestful endpoints, e.g.: `/file.{extension}`
