...
### Explanation
Velonetics can run either as a full API gateway or as a transparent proxy using [no-op encoding](/docs/v2.12/endpoints/no-op/). When set as a proxy, Velonetics faithfully returns backend headers, status codes (including 204), and response bodies, but most gateway features like circuit breakers are inactive since Velonetics does not modify or interpret the response.
