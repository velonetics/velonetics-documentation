...
| -------- | ------- | ------- |
| {{< badge color="#6f00f0">}}server{{< /badge >}}      | Router | [HTTP server plugins](/docs/v2.12/extending/http-server-plugins/) modify the HTTP request and response between the end-user and Velonetics (the server)    |
...
| {{< badge color="#e900b7">}}client{{< /badge >}}      | Transport | [HTTP client plugins](/docs/v2.12/extending/http-client-plugins/) modify the request and response between Velonetics and the upstream services (internal HTTP client)    |
...
Before compiling the plugin, you must ensure the libraries you use in your code are compatible with Velonetics. To do so, execute the command [`{{< product check_plugin_command >}}`](/docs/v2.12/extending/check-plugin/) that analyzes your `go.sum` file and warns you about incompatibilities.
...
The command will generate a `yourplugin.so` file (name it as you please) that you can now copy into a `{{< product image >}}:2.12` Docker image, and load it as described in [injecting plugins](/docs/v2.12/extending/injecting-plugins/). **Never use a tag that mismatches the builder and Velonetics**. If you want to load the plugin in a Velonetics version `x.y.x` make sure to build it on a builder `x.y.z`. Using `.so` files that were compiled in a builder with a different version, will mostly fail.
...
## Test the plugin
Once your `.so` file is available, you must check that the plugin is loadable by Velonetics. You can test the plugin using the [test command](/docs/v2.12/extending/test-plugin/)
