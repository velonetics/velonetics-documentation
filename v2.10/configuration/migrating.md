...
2. **Adjust your configuration and run the linter** as needed. Below, you'll find the changes between versions. Scroll down to your current version and apply all changes above it. Then, run [the linter](/docs/v2.10/configuration/check/) (`velonetics check --lint`), which is designed to scrutinize your configuration rigorously.
...
**If you have custom go plugins**, recompile them. Velonetics has now a command [`velonetics check-plugin`](/docs/v2.10/extending/check-plugin/) and [`velonetics test-plugin`](/docs/v2.10/extending/test-plugin/) to test them.
