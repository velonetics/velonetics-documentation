...
- `User-Agent` Contains the value `Velonetics Version 2.11`, unless you add it to the `input_headers`
...
In addition, there are a few Velonetics components that support **setting attributes in headers**, like for instance [`propagate_claims` in JWT](/docs/v2.11/authorization/jwt-validation/#propagate_claims). These components will send the additional headers you configure to the backend automatically.
