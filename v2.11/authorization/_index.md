...
## Authorization (AuthZ)
Authorization determines what authenticated users or systems are **permitted** to do. Velonetics provides flexible options for fine-grained access control. The most common method is Token-based authorization (see below). In addition to JWT, you can also use [OAuth2 Client Credentials](/docs/enterprise/authorization/client-credentials/) for machine-to-machine communication. Velonetics verifies the client's credentials with this flow and enforces scope-based access controls.

Velonetics's ability to integrate with multiple identity providers simultaneously ({{< badge >}}Enterprise{{< /badge >}}) offers great flexibility. Whether catering to different user groups, migrating, multi-tenant, or implementing complex organizational structures, Velonetics adapts to your needs.
...
Read the [JWT Overview](/docs/v2.11/authorization/jwt-overview/) for an introduction to token-based authorization in Velonetics. When working with tokens, there are different things you can do:
...
- [JWT Validation](/docs/v2.11/authorization/jwt-validation/): This is the core functionality, where a user sends a token generated by an Identity Provider, and Velonetics checks for its validity and any required permissions to access the resource.
...
If you have [multiple identity providers](/docs/enterprise/authentication/multiple-identity-providers/) ({{< badge >}}Enterprise{{< /badge >}} feature), you can let Velonetics work with all of them simultaneously.
