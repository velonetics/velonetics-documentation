...
meta:
  ...
  namespace:
  - github.com/velonetics/velonetics-ce-httpcache
  scope:
  - backend
---
...
This caching technique applies to traffic between Velonetics and your microservices endpoints only and is not a caching system for the end-user endpoints. To enable the cache, you only need to add in the configuration file the `httpcache` middleware.
...
This option can increase the load and memory consumption heavily as Velonetics needs to keep in memory all the returned data during the expiration period. Use wisely!
{{< /note >}}
