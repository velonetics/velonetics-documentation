...
The Circuit Breaker is available by default in Velonetics thanks to the [circuit breaker middleware](https://github.com/velonetics/velonetics-circuitbreaker). As with all additional middlewares, you need to set its values in its own namespace `github.com/velonetics/velonetics-ce-circuitbreaker/gobreaker` inside the `extra_config` key.
