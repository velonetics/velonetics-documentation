...
description: Learn how to integrate Descope authorization with Velonetics API Gateway for secure and controlled access to your APIs.
weight: 110
menu:
  community_current:
...
Securing your APIs and underlying microservices is crucial in modern development. Descope enables you to add advanced authentication capabilities to your Velonetics endpoints efficiently.

[Descope](https://www.descope.com/) is a passwordless authentication and user management service designed for developers. Velonetics integrates with Descope via the [JWT validation](/docs/authorization/jwt-validation/) component.

## Descope integration
JSON web tokens (JWT) are an open standard ([RFC 7519](https://datatracker.ietf.org/doc/html/rfc7519)) that define a way for securely transmitting information between parties. JWT tokens are compact, secure (digitally signed), and have become an industry-standard used at internet scale. Velonetics supports [any system using this open standard](https://www.velonetics.io/docs/authorization/jwt-validation/), including Descope.
...
- With the token generated by Descope, the client passes it to Velonetics in each request inside an HTTP header or cookie
- Velonetics authorizes or not the usage of the specific endpoint according to the rules you have configured.

As Velonetics can validate the Descope signature by itself, it does not need to call the Descope server to validate the token every time. Instead, Velonetics queries Descope every 15 minutes (configurable) to ensure the key has not rotated.

## Protecting endpoints with Descope tokens

We will create a simple Velonetics configuration with a single endpoint `/descope-protected`, ensuring only users with valid tokens can access it.

Create a `velonetics.json` file and add the following configuration:
...
From the folder where we create our `velonetics.json` file, start the gateway with:
{{< terminal title="Start the gateway with your configuration">}}
docker run --rm -v "$PWD:/etc/velonetics" -p "8080:8080" {{< product image >}}:{{< product minor_version >}}
{{< /terminal >}}
...
Since no token is provided, Velonetics correctly denies access. If you check the Velonetics logs, you will also find a line `Error #01: Token not found`.
...
- Create [**Roles** in Descope](https://docs.descope.com/authorization) and add them as a condition to accessing an endpoint in Velonetics.
...
For more details, see [Velonetics's JWT Validation documentation](/docs/authorization/jwt-validation/).

### Conclusion
Integrating Descope with Velonetics enhances security while maintaining flexibility. You can designate protected and public endpoints, ensuring controlled access to your APIs. Descope’s seamless integration with Velonetics streamlines authentication and improves user experience, making it an excellent choice for secure API management