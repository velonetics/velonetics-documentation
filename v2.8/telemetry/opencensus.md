...
Velonetics has traditionally offered its telemetry integration through this **OpenCensus** component, which has provided reliable service for over six years, but now is transitioning to the more modern and robust [OpenTelemetry](/docs/v2.8/telemetry/opentelemetry/) framework.
...
If you have been using telemetry on Velonetics for the past six years (before Velonetics 2.6), you were using the [OpenCensus](/docs/v2.8/telemetry/opencensus/) exporters, which have worked like a Swiss clock.
...
While our OpenCensus integration will keep functioning on Velonetics for now, it won't receive additional updates (netiher security fixes), so we recommend replacing it with OpenTelemetry.