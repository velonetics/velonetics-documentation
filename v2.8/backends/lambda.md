...
Notice in the examples that the `host` and `url_pattern` are needed as per the [backend definition](/docs/v2.8/backends/), but Velonetics will never use them when connecting to a Lambda. Feel free to add any value in there, but the entry must be present.
{{< /note >}}
...
docker run --rm -it -p "8080:8080" \
    ...
    -v "$PWD:/etc/velonetics" {{< product image >}}:2.8
{{< /terminal >}}
