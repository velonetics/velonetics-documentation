...
- **Same Go version**: Your plugin and Velonetics are compiled with the same Go version. E.g., you cannot build a plugin on Go 1.19 and load it on a Velonetics assembled with Go 1.22.
- **Same architecture/platform**: Velonetics and your plugin must have been compiled in the same architecture. E.g., you cannot compile a plugin in a Mac and use it in a Docker container (use the builder, see below)
...
As your custom plugins need to match the Go and libraries versions used to build Velonetics, you have to guarantee your plugin is compatible by checking the `go.sum` file with the command `check-plugin` ([read the documentation](/docs/v2.8/extending/check-plugin/))
...
Now [test it](/docs/v2.8/extending/test-plugin/), or [load it](/docs/v2.8/extending/injecting-plugins/) in Velonetics
