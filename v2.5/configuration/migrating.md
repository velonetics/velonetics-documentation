...
title: Migrating Velonetics Configuration from v1.x or v0.x
description: Seamlessly migrate your Velonetics configuration to a new environment. Hassle-free transition for your API Gateway
menu:
  community_v2.5:
...
The Velonetics 2.0 release is a major version that **simplifies the configuration** of `v1.x` and **standardizes field names** that were using different criteria to declare the attributes.

This migration allows you to:

- Migrate from Velonetics `0.x` to `2.x`
- Migrate from Velonetics `1.0` to `2.x`
- Migrate from Velonetics `1.2` to `2.x`
- Migrate from Velonetics `1.3` to `2.x`
- Migrate from Velonetics `1.4` to `2.x`
...
## Migrating to Velonetics 2.0 and above
...
**If you have custom go plugins**, recompile them. Velonetics has now a command [`velonetics check-plugin`](/docs/v2.5/extending/check-plugin/) to test them.
...
### Renamed namespaces
The most visible change is that all non-core components (this is everything outside of [Lura](https://luraproject.org)) were declared inside an `extra_config` section, using a looong **namespace**. That namespace contained what it could look like an URL (e.g: `github.com/velonetics/velonetics-ce-jose/validator`) and was generating frequent missunderstanding year after year. Now all nampespaces have been categorized and simplified to a description of its functionality (e.g.: `auth/validator`).
