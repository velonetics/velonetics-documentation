...
- `$schema`: *Optional*. When added, enables [IDE integration](/docs/v2.5/developer/ide-integration/) with autocompletion and documentation. Defines the JSON schema to validate your configuration. Is used by ` velonetics check --lint`.
...
For instance, you might want to set a [rate limit](/docs/v2.5/throttling/) between a user and a `/my-rate-limited` endpoint in Velonetics. And for that, you would place the `extra_config` inside that `endpoint` scope. Or you might want to limit the connections between a Velonetics endpoint against your services; then you would place the `extra_config` in the `backend` scope.
...
For larger sample files with more options you can have a look a the [Velonetics Playground](/docs/v2.5/overview/playground/).
