...
As your custom plugins need to match the Go and libraries versions used to build Velonetics, you have to guarantee your plugin is compatible by checking the `go.sum` file with the command `check-plugin` ([read the documentation](/docs/v2.5/extending/check-plugin/))
...
Now load it in Velonetics, as described in [injecting plugins](/docs/v2.5/extending/injecting-plugins/)