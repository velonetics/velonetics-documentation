...
The `velonetics check` command **validates Velonetics configuration files** written in any of its [supported formats](/docs/v2.6/configuration/supported-formats/).
...
- `-l` or `--lint` (*optional*) to check that your configuration file is properly linted and does not contain unrecognized options or wrong types. This option requires Internet access as the schema is downloaded from `https://www.velonetics.io/schema/v2.6/velonetics.json` (each version uses its schema).
