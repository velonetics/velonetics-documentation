...
3) **If you don't have an identity server**, you can still use your classic monolith/backend login system and adapt it to return a JWT payload (which is a simple JSON). From here, let Velonetics [sign the token for you](/docs/v1.4/authorization/jwt-signing/) and start using tokens right away.
...
These options are for the `extra_config`'s namespace `"github.com/velonetics/velonetics-ce-jose/validator"` placed in every endpoint (use [flexible configuration](/docs/v1.4/configuration/flexible-config/) to avoid code repetition):
...
The [Velonetics Playground](/docs/v1.4/overview/playground/) demonstrates how to protect endpoints using JWT and includes two examples ready to use:
