...
When a Velonetics endpoint is hit, the engine requests **all defined backends in parallel** (unless a [sequential proxy](/docs/v1.4/endpoints/sequential-proxy/) is used). The returned content is parsed according to its `encoding` or middleware configuration.
...
- `encoding`: Define your [needed encoding](/docs/v1.4/backends/supported-encodings/) to inform Velonetics how to parse the response.
