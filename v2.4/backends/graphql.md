...
With the example and the configuration of Velonetics above, when the user sends a body, it will be sent as it is to the backend. However, if the user does not include any of the `variables` in the body, it will add them to the final request to the backend. So, with this example, any `review` will receive 3 stars and a "meh" comment if the end-user does not pass it.
...
{{< terminal title="Query to graphql endpoint" >}}
curl -G http://velonetics/review/5
{{< /terminal >}}
...
{{< terminal title="Query to graphql endpoint" >}}
curl -XPOST -d '{"episode": "JEDI"}' http://velonetics/hero
{{< /terminal >}}
...
It is a simplistic approach but still very powerful, as you can define templates with queries and let velonetics aggregate the responses.
