...
Velonetics implements both [JWT signing](/docs/v2.4/authorization/jwt-signing/) and [JWT validation](/docs/v2.4/authorization/jwt-validation/) models to protect endpoints from undesired users that are not entitled to use the information, reinforcing security.

- [Sign tokens](/docs/v2.4/authorization/jwt-signing/) when you have no identity server yet (like a classic monolithic application with a `/login` endpoint) and let Velonetics take care of the token signing with the private key.
