...
## Plugin Builder
When using Docker to deploy your gateway, the official Velonetics container uses **[Alpine](https://hub.docker.com/_/alpine)** as the base image. Therefore, to use your custom plugins, they must compile within an Alpine container and use the same Go and Glibc versions as Velonetics. The **Plugin Builder** docker image spares you from this job.
...
As your custom plugins need to match the Go and libraries versions used to build Velonetics, you have to guarantee your plugin is compatible by checking the `go.sum` file with the command `check-plugin` ([read the documentation](/docs/v2.4/extending/check-plugin/))
...
Now load it in Velonetics, as described in [injecting plugins](/docs/v2.4/extending/injecting-plugins/)

## Debugging plugins in an IDE
If you'd like to debug your plugins with an IDE, you can enable **delve flags** when compiling. Velonetics must be compiled with these gcflags too. This option is not available on the Enterprise product.
