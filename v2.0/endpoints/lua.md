...
A [Go plugin](/docs/v2.0/extending/) delivers much more speed and power than a Lua script for performance-first seeking users, but requires a little bit more work as you need to compile your plugins and side-load them on Velonetics.
{{< /note >}}
...
- `sources`: An array with all the external files that Velonetics will include in the first place. You can define the functions in external files and refer them on `pre` or `post`.
...
- `live`: Live reload of the script in every execution. Set to `true` if you intend to modify the Lua script while Velonetics is running (mostly during development)
...
- **Router** (at `endpoint`'s `extra_config`): Communication between the end-user and Velonetics. You can inspect and modify the **request** of the user.
...
If you have a script that needs access to the request, use the `request` object in Lua. The request is set when Velonetics is about to do a call to the backend services. The `request` functions are:
...
*   `url(value)` (_Dynamic_): Setter that changes the url of the request. E.g.: `c:url('http://domain.com/api/test')`. Changing the value before the `url` is calculated will result in Velonetics overwriting its value.
