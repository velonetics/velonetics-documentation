...
When the service starts, **Velonetics doesn't know the type of plugin** you have coded until it tries to register it, and **it will try to register it as all known types**. When using a `DEBUG` log level you will see this activity in the logs.
...
For example, let's see how loading three different plugins are logged into Velonetics:
