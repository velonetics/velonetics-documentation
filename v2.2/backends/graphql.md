...
![GraphQL](/images/documentation/graphql/velonetics-rest-to-graphql-transformation.png)

**Velonetics can use the variables in the body or in the endpoint URL** to generate the final GraphQL query that will be sent to the GraphQL server. The query is loaded from an external file or declared inline in the configuration and contains any variables needing replacement with the user input.

**Velonetics acts as the GraphQL client**, negotiating with the GraphQL server the content and hiding its complexity to the end-user. The end-user consumes REST content and retrieves the data in JSON, XML, RSS, or any other format supported by Velonetics.
...
- `POST`: The query to the GQL server uses an **autogenerated body** containing all the variables of the URL parameters **OR** the request body. When the user and the Velonetics configuration define the same variables (collision) the user variables take preference.
...
### POST + mutation
Suppose the end-user makes the following request to Velonetics, which contains a body with a JSON containing `campaign` information to `/marketing/abcdef`:

{{< terminal title="Query to graphql endpoint" >}}
curl -XPOST -d '{"campaign": "xmas"}' http://velonetics/marketing/abcdef
{{< /terminal >}}

With the example and the configuration of Velonetics above, Velonetics combines the variables you have defined in the configuration with the ones sent in the request and the GraphQL server receives a body like the following:
...
{{< terminal title="Query to graphql endpoint" >}}
curl -XGET http://velonetics/marketing/abcdef
{{< /terminal >}}
...
![Graphql](/images/documentation/graphql/velonetics-graphql-proxy.png)
...
It is a simplistic approach but still very powerful, as you can define templates with queries and let velonetics do the aggregation of the responses.
...
The following example is a REST endpoint consuming data from 2 different subgraphs in parallel. You could add here any other Velonetics components you could need:
