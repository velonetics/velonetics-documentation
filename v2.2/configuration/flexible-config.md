...
- `FC_OUT=file.json`: Saves the resulting configuration after rendering the template. It's required when you don't use JSON content (e.g., `FC_OUT=velonetics.yml`), or when you need to pass the output to another program (like [`check --lint`](/docs/v2.2/configuration/check/)).
...
Save the following `docker-compose.yml` and do a `docker compose up`. This setup with the [`:watch` image](/docs/v2.2/developer/hot-reload/) will allow you to work locally with the latest version of Velonetics and apply the changes automatically every time you change a source file.
