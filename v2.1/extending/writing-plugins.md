...
The command will generate a `yourplugin.so` file (name it as you please) that you can now load in Velonetics, as described in [injecting plugins](/docs/v2.1/extending/injecting-plugins/)


## Compiling plugins without Docker
As your custom plugins need to match the Go and libraries versions used to build Velonetics, you have to guarantee your plugin is compatible by checking the `go.sum` file with the command `check-plugin` ([read the documentation](/docs/v2.1/extending/check-plugin/))
...
Now load it in Velonetics, as described in [injecting plugins](/docs/v2.1/extending/injecting-plugins/)